// Edge Function : vérifie le statut réel d'une transaction (appelée par la page
// de confirmation). Le billet n'est renvoyé que si le paiement est ACCEPTED.
// Déployer avec verify_jwt = false.
import { corsHeaders, json } from '../_shared/cors.ts';
import { admin, refreshStatus } from '../_shared/fedapay.ts';
...
Deno.serve(async (req) => {
  ...
  return json({
    ...
    payment: { key: 'fedapay', label: 'FedaPay' },
    customer: {
